...
export default async function Dashboard () {
    ...
    const stockSymbols = ['NFLX', 'AAPL', 'AMZN', 'GOOGL', 'MSFT', 'TSLA'];
    const stockData = await Promise.all(stockSymbols.map(symbol => yahooFinance.quote(symbol)));
    ...
    return ( 
      ...
      {cryptoResults ? (
        <div className=" bg-gradient-to-tl from-blue-900 to-black text-white pb-10 pt-24 flex flex-col justify-center items-center gap-5 px-24 max-[900px]:px-10 max-[752px]:px-5 ">
            ...
            {/* Nouvelle section pour les investissements en actions */}
            <div className="w-full mt-10">
                <h3 className="text-xl">My Stock Investment</h3>
                <p className="text-[12px] text-slate-500">Track your stock investments</p>
                <TableContainer>
                    <Table size="small" aria-label="Stock Investment Table">
                        <TableHead>
                            <TableRow className="text-white">
                                <TableCell className="text-white">#</TableCell>
                                <TableCell className="text-white">Company</TableCell>
                                <TableCell className="text-white">Shares</TableCell>
                                <TableCell className="text-white">Current Price</TableCell>
                                <TableCell className="text-white">Total Value</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody className="text-white">
                            {stockData.map((stock, index) => (
                                <TableRow key={stock.symbol}>
                                    ...
                                    <TableCell className="text-white">{stock.shortName}</TableCell>
                                    <TableCell className="text-white">N/A</TableCell>
                                    <TableCell className="text-white">${stock.regularMarketPrice}</TableCell>
                                    <TableCell className="text-white">N/A</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
            ...
            <div className="w-full mt-10">
                <h3 className="text-xl">My Investment Horizon</h3>
                <p className="text-[12px] text-slate-500">Plan your investment timeline</p>
                <TableContainer>
                    <Table size="small" aria-label="Investment Horizon Table">
                        <TableHead>
                            <TableRow className="text-white">
                                <TableCell className="text-white">#</TableCell>
                                <TableCell className="text-white">Investment</TableCell>
                                <TableCell className="text-white">Horizon</TableCell>
                                <TableCell className="text-white">Expected Profit</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody className="text-white">
                            {stockData.map((stock, index) => (
            ...
            <div className="w-full mt-10">
                <h3 className="text-xl">Expected Profits</h3>
                <p className="text-[12px] text-slate-500">Estimate your potential earnings</p>
                <TableContainer>
                    <Table size="small" aria-label="Expected Profits Table">
                        <TableHead>
                            <TableRow className="text-white">
                                <TableCell className="text-white">#</TableCell>
                                <TableCell className="text-white">Investment</TableCell>
